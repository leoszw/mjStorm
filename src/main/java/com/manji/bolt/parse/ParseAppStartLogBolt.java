 ...
 * Description: app启动事件解析
 * User: szw
 * Date: 2019-09-27
 * Time: 15:01
 */
public class ParseAppStartLogBolt implements IRichBolt {
...
//            //公共属性
            ...
            registerTime = HashMapUtil.getStrFromHashMap("registerTime", hashMap);
            screenName = HashMapUtil.getStrFromHashMap("screenName",hashMap);
            title = HashMapUtil.getStrFromHashMap("title",hashMap);

            //发送数据
            collector.emit(new Values(timestamp, hour, days, week, month, quarter, year, appName, event, version, province, userID,
                    userGroup, brand, model, networkType, os, resolutionRatio, distinctId, registerTime,screenName,title));
        }
    }

    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declare(new Fields("timestamp", "hour", "days", "week", "month", "quarter", "year", "appName", "event", "version", "province", "userID",
                "userGroup", "brand", "model", "networkType", "os", "resolutionRatio", "distinctId", "registerTime","screenName","title"));
    }
